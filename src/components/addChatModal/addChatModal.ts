  ...
  constructor(props?: TProps) {
    super('div', {
      ...
      btnCreateChat: new Button({
        ...
        events: {
          click: async () => {
            let labelError = document.querySelector('.modal-error') as HTMLElement;
            if (labelError) {
              labelError.style.display = 'none';
            }
            const titleChat = (document.querySelector('#title-chat') as HTMLInputElement).value;
            const loginToInvite = (document.querySelector('#login-to-invite') as HTMLInputElement)
              .value;
            const avaChat = document.querySelector('#ava-chat') as HTMLInputElement;

            if (titleChat !== '' && loginToInvite !== '' && avaChat.files?.length !== 0) {
              await userController.getByLogin(loginToInvite).then(async (resp) => {
                ...
                if (user) {
                  await chatsController.createChat(titleChat).then(async (chat) => {
                    ...
                    form.append('avatar', document.querySelector('#ava-chat').files[0]);
